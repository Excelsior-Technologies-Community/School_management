...
import { School, UserPlus, LogOut, ShieldCheck, Clock, Mail, MapPin, RefreshCw, List, ChevronLeft, ChevronRight, Search, Layers, PlusCircle, Tag, Check, ToggleLeft, ToggleRight, X } from 'lucide-react';
...
const SuperAdminDash = () => {
  ...
  const fetchMasterSubjectData = async () => {
    ...
    try {
      ...
      console.error('Subject data loader crash:', error);
    ...
    try {
      ...
      if (response.data.success) {
        if (action === 'rejected') {
          ...
          // If approved, update the item locally or trigger your state refresh callback
          setPendingRequests(prev => prev.filter(req => req.master_subject_id !== masterSubjectId));
          // optional reload hook: fetchMasterSubjects();
        }
          ...
          <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 items-start'>
            <div className='bg-white rounded-2xl border border-slate-200 p-6 shadow-sm lg:sticky lg:top-24'>
              ...
              <form onSubmit={handleSubjectSubmit} className='space-y-4'>
                {/* ... (Your existing Form Inputs remain exactly the same) ... */}
                <div>
